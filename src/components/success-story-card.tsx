...
export function SuccessStoryCard({
  ...
  const growth = before_price > 0 && Number.isFinite(before_price) && Number.isFinite(after_price) ? ((after_price - before_price) / before_price) * 100 : null;
      ...
      <CardContent className="p-6">
        <div>
          <div className="flex items-start justify-between mb-3">
            ...
            {growth !== null && <div className="flex items-center gap-1 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 rounded-lg px-2.5 py-1">
              <TrendingUp className="size-3.5" />
              <span className="text-xs font-bold">{growth > 0 ? "+" : ""}{Math.round(growth)}%</span>
            </div>}
          </div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-2 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
            Valores informados pelo profissional. Resultados individuais podem variar.
          </p>
        </div>
      </CardContent>
    </Card>
