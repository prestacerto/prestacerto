...
export function ProposalForm({
  ...
  async function handleImprove() {
    ...
    try {
      ...
      if (!res.ok) {
        setAiUpgrade(body?.upgrade === true);
      ...
      setAiRemaining(body.remainingFree ?? null);
  ...
  async function onSubmit(values: ProposalValues) {
    ...
    setSent(true);
    trackAnalyticsEvent("proposal_submitted", { project_id: projectId, has_price: Boolean(price && price > 0) });
    toast.success("Proposta enviada!");
    router.refresh();
    } catch {
      toast.error("Não foi possível conectar. Seu texto continua no formulário; tente novamente.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-1.5">
        ...
        <p className="text-[11px] text-slate-500">Certo AI: 3 solicitações gratuitas por mês. Pedidos enviados podem consumir a cota mesmo se a conexão cair.</p>
        {aiRemaining !== null && <p className="text-xs text-slate-600">Solicitações gratuitas restantes: {aiRemaining}</p>}
        {aiUpgrade && <Link href="/plans" className="block text-xs font-semibold text-blue-600 underline">Conhecer o plano Pro</Link>}
      </div>
