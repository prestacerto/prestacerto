...
export function GoogleAuthButton() {
  ...
  async function handleClick() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/callback?${new URLSearchParams({ next: safeDestination(searchParams.get("next") || searchParams.get("redirect")) })}`,
      },
    });
    if (error) {
      toast.error("Não foi possível continuar com o Google", {
        description: "Use seu e-mail e senha ou tente novamente em instantes.",
      });
      setLoading(false);
    }
