...
export async function optimizeProposal(proposal: string, context?: { category?: string; budget?: number; metering?: Omit<AiMeteringContext, "product"> }): Promise<ProposalAnalysis> {
  try {
...
Retorne em JSON:
{
  ...
  "feedback": "resumo das principais mudanças"
    ...
    await recordAiUsage({ provider: "anthropic", model: message.model, requestId: message.id, usage: anthropicUsage(message), context: { ...context?.metering, product: "prestacerto" }, metadata: { feature: "proposal-optimize" } });
...
export async function generateProposalTips(metering?: Omit<AiMeteringContext, "product">): Promise<string[]> {
  ...
  await recordAiUsage({ provider: "anthropic", model: message.model, requestId: message.id, usage: anthropicUsage(message), context: { ...metering, product: "prestacerto" }, metadata: { feature: "proposal-tips" } });
...
Retorne JSON:
{
  ...
  await recordAiUsage({ provider: "anthropic", model: message.model, requestId: message.id, usage: anthropicUsage(message), context: { ...metering, product: "prestacerto" }, metadata: { feature: "proposal-compare" } });
