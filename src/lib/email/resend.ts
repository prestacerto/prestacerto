...
export async function sendContactNotificationEmail(params: {
  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        <p><strong>${escape(params.name)}</strong> (${escape(params.email)}) mandou uma mensagem pelo site:</p>
        <p style="white-space: pre-line; border-left: 3px solid #e2e8f0; padding-left: 12px;">${escape(params.message)}</p>
      </div>
    `,
  });
