...
import { getPublicCategories } from "@/lib/supabase/categories";
import { SEARCH_PAGE_SIZE, searchTextFilter } from '@/lib/search-pagination';
...
export async function listServices(params: {
  ...
  try {
    ...
    if (params.query) {
      request = request.or(searchTextFilter(params.query));
    }

    if (params.page !== undefined) {
      const offset = (Math.min(10000, Math.max(1, Math.floor(params.page) || 1)) - 1) * SEARCH_PAGE_SIZE;
      request = request.range(offset, offset + SEARCH_PAGE_SIZE);
    }
...
export async function getServiceById(id: string) {
  try {
    ...
    const { data, error } = await supabase
      .from("services")
      .select(
        "*, freelancer:profiles!services_freelancer_id_fkey(id, full_name, city, state, bio, avatar_url, plan)"
      )
      .eq("id", id)
      .single();
    if (error) throw error;
...
export async function getPublicProfile(freelancerId: string) {
  try {
    ...
    const { data: freelancer, error } = await supabase
      .from("profiles")
      .select("id, full_name, avatar_url, bio, city, state, plan, role")
      .eq("id", freelancerId)
      .single();
...
export async function getPublicPortfolioBySlug(slug: string) {
  try {
    ...
    return {
      ...
      stats: {
        reviews_count: freelancer.rating_count ?? 0,
...
export async function getReviewsForProfile(profileId: string): Promise<PublicReview[] | null> {
  try {
    // The raw table is private. Only these fields are intentionally public.
    const supabase = createServiceClient();
    const { data, error } = await supabase.from("reviews")
      .select("id, rating, comment, created_at").eq("target_id", profileId)
      .order("created_at", { ascending: false }).limit(50);
    if (error) throw error;
    return (data ?? []).map(({ id, rating, comment, created_at }) => ({ id, rating, comment, created_at }));
  } catch (error) {
...
export async function getMyReviewForProject(projectId: string, userId: string): Promise<boolean | null> {
  try {
    const supabase = await createClient();
    const { data, error } = await supabase.from("reviews").select("id")
      .eq("project_id", projectId).eq("author_id", userId).maybeSingle();
    if (error) throw error;
    return Boolean(data);
  } catch (error) {
...
export async function listOpenProjects(params: {
  ...
  try {
    ...
    let request = supabase
      .from("projects")
      .select(
        "id, title, description, skills, budget_min, budget_max, deadline_days, status, category_id, created_at"
      )
...
export async function getMyProjects(userId: string) {
  const supabase = await createClient();
  const { data, error } = await supabase.from("projects").select("id, title, status, category_id, created_at, proposals(count)").eq("client_id", userId).order("created_at", { ascending: false });
  if (!error) return (data ?? []).map(project => ({ ...project, proposal_count: Array.isArray(project.proposals) ? (project.proposals[0]?.count ?? 0) : 0 }));
  // A missing proposal grant must not hide projects that belong to this account.
  const fallback = await supabase.from("projects").select("id, title, status, category_id, created_at").eq("client_id", userId).order("created_at", { ascending: false });
  if (fallback.error) { console.error("getMyProjects: projects unavailable"); return []; }
  return (fallback.data ?? []).map(project => ({ ...project, proposal_count: null as number | null }));
}

export async function getProjectSharedFolder(projectId: string) {
  const supabase = await createClient();
  const { data, error } = await supabase.from("project_google_data").select("drive_folder_url").eq("project_id", projectId).maybeSingle();
