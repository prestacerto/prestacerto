...
export async function exchangeNuvemshopCode(code: string): Promise<NuvemshopTokenResponse> {
  // No Partner Portal, o identificador usado como client_id é o App ID.
  ...
  if (!clientId || !clientSecret || !siteUrl) {
    throw new Error("NUVEMSHOP_APP_ID, NUVEMSHOP_CLIENT_SECRET/NEXT_PUBLIC_SITE_URL não configurados");
  }

  const redirectUri = `${siteUrl.replace(/\/$/, "")}/api/integrations/nuvemshop/callback`;
