 ...
 */
export async function analyzeScopeFromMessage(
  messageText: string,
  metering?: Omit<AiMeteringContext, "product">,
...
Responda APENAS em JSON, sem explicações:
{
  ...
  try {
    ...
    await recordAiUsage({
      ...
      context: { ...metering, product: "prestacerto" },
      metadata: { feature: "whatsapp-scope", ...metering?.metadata },
    });
