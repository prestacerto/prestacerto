...
export const CATEGORIAS: Record<string, { label: string; labelPlural: string; descricao: string }> = {
  ...
  psicologia: {
    ...
    descricao: "atendimento psicológico, terapia e orientação",
  },
};
...
// Route parameters must match catalog entries, never Object.prototype keys.
export function getLandingCity(slug: string) {
  return Object.hasOwn(CIDADES, slug) ? CIDADES[slug] : undefined;
}

export function getLandingCategory(slug: string) {
  return Object.hasOwn(CATEGORIAS, slug) ? CATEGORIAS[slug] : undefined;
}
