...
import { CheckCircle2, Calendar, HardDrive, Store, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="mx-auto w-full max-w-2xl space-y-8">
      ...
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center gap-2">
            <span aria-hidden className="flex size-5 items-center justify-center rounded bg-[#0A66C2] text-xs font-bold text-white">in</span>
            <span className="font-semibold">LinkedIn</span>
          </div>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Compartilhe seu perfil público no LinkedIn para mostrar seus serviços e facilitar novas oportunidades.
          </p>
          <div className="flex flex-wrap gap-2">
            <Link
              href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${process.env.NEXT_PUBLIC_APP_URL || 'https://prestacerto.com.br'}/perfil/${user.id}`)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-lg bg-[#0A66C2] px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-[#004182]"
            >
              <span aria-hidden className="text-xs font-bold">in</span>
          ...
          <p className="text-xs text-muted-foreground">
            A publicação usa o compartilhamento oficial do LinkedIn e não solicita senha nem token nesta etapa.
          </p>
        </CardContent>
      </Card>
    </div>
