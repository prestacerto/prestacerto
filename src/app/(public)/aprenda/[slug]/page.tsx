...
export async function generateMetadata({
  ...
  if (!card) notFound();
  const base = getPageMetadata(`${card.titulo} | Aprenda`, describePage(card.conteudo, card.titulo), `/aprenda/${card.id}`);
    ...
    <article className="mx-auto max-w-2xl px-4 py-12">
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: serializeJsonLd({
            ...
            author: { '@type': 'Organization', name: 'PrestaCerto', url: siteUrl },
            mainEntityOfPage: { '@type': 'WebPage', '@id': `${siteUrl}/aprenda/${card.id}` },
            inLanguage: 'pt-BR',
          }),
        }}
      />
      ...
      <div className="mt-12 rounded-2xl border bg-gradient-to-br from-blue-50 to-transparent p-6 dark:from-blue-950/40">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Crie seu perfil e apresente seu trabalho no PrestaCerto. A plataforma não cobra comissão sobre o serviço.
        </p>
