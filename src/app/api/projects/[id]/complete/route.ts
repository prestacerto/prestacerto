...
export async function POST(
  ...
  try {
    ...
    // Explicit launch mode: payment is agreed directly, with no platform capture.
    if (process.env.MARKETPLACE_PAYMENT_MODE === "direct") {
      const { data: completed, error } = await supabase.from("projects")
        .update({ status: "closed" }).eq("id", params.id).eq("client_id", user.id)
        .eq("status", "in_progress").select("id").maybeSingle();
      if (error || !completed) return NextResponse.json({ error: "Não foi possível concluir o projeto agora." }, { status: 409 });
